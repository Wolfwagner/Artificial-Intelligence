...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for location in neighboring_grid_locs:
            nbr_states.append(SingleGoalGridState(location, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors))
    ...
    def compute_heuristic(self):
        heuristic = abs(self.state[0] - self.goal[0][0]) + abs(self.state[1] - self.goal[0][1])
        return heuristic
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for location in neighboring_locs:
            nbr_states.append(GridState(location, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache))
    ...
    def compute_heuristic(self):
        ...
        if self.goal in self.mst_cache:
             mst_cost = self.mst_cache[self.goal]             # Check if MST value is cached
        else:
             mst_cost = compute_mst_cost(self.goal, manhattan)        #  Compute MST cost and cache it
             self.mst_cache[self.goal] = mst_cost

        return manhattan(self.state, nearest_goal) + mst_cost
